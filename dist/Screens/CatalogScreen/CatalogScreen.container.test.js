...
jest
    ...
    __esModule: true,
    default: (props) => MockCatalogScreenComponent(props),
}));
...
describe('CatalogProductScreenContainer Container', () => {
    ...
    beforeEach(() => {
        jest.clearAllMocks();
        useCatalogProductInfinite.mockReturnValue(mockHookReturnValue);
    });
    it('should render CatalogProductScreenComponent and pass correct mapped props', async () => {
        await render(_jsx(CatalogProductScreenContainer, { route: mockRoute, navigation: mockNavigation }));
        expect(MockCatalogScreenComponent).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('should handle fallback title and empty products gracefully when data is undefined', async () => {
        useCatalogProductInfinite.mockReturnValue({
            ...mockHookReturnValue,
            data: undefined,
        });
        ...
        await render(_jsx(CatalogProductScreenContainer, { route: routeWithoutParams, navigation: mockNavigation }));
        expect(MockCatalogScreenComponent).toHaveBeenCalledWith(expect.objectContaining({
            title: undefined,
            products: [],
        }));
