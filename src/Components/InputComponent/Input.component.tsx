...
import type {
  InputComponentProps,
  PropsLoadingIcon,
  PropsRenderTextInput,
  PropsRenderSuggestionItem,
  RenderActionButtonProps,
  RenderSuggestionListProps,
...
const _renderLabel = (
  label: string,
  labelStyle: Record<string, Animated.Interpolation<number | string>>,
): React.ReactNode => (
...
const _renderTextInput = ({
  ...
  handleFocus,
  ...restProps
}: PropsRenderTextInput): React.ReactNode => (
  <TextInput
    editable={!disabled}
...
}: RenderActionButtonProps): React.ReactNode => (
 ...
 */
const _renderSuggestionItem = ({
  item,
  onSelectSuggestion,
  setIsFocused,
}: PropsRenderSuggestionItem): React.ReactElement => (
  <TouchableOpacity
    key={item.id}
    activeOpacity={0.7}
 ...
 */
const _renderSuggestionList = ({
  showSuggestions,
  visibleSuggestions,
  onSelectSuggestion,
  setIsFocused,
}: RenderSuggestionListProps): React.ReactNode => {
  if (!showSuggestions) return null;

  return (
    <View style={styles.suggestionDropdown}>
      {visibleSuggestions.map((item) =>
        _renderSuggestionItem({ item, onSelectSuggestion, setIsFocused }),
      )}
    </View>
  );
};
 ...
 */
const _renderErrorText = (error?: string): React.ReactNode => {
...
const useInput = ({
  ...
  onBlur,
  suggestions = [],
}: InputComponentProps): HooksInputReturnType => {
  ...
  const labelStyle: Record<string, Animated.Interpolation<number | string>> =
    _labelStyle(animatedValue, activeColor, inactiveColor);
