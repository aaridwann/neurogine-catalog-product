...
describe('useCatalogProductInfinite Hook', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    (useInfiniteQuery as jest.Mock).mockReturnValue(mockInfiniteQueryResult);
  });

  it('should initialize and return query result along with handlers', async () => {
    const { result } = await renderHook(() =>
      useCatalogProductInfinite(mockNavigation, { search: 'test' }),
    );
  ...
  it('should configure useInfiniteQuery with correct queryKey and getNextPageParam logic', async () => {
    ...
    expect(queryConfig.queryKey).toEqual(
      catalogKeys.list({ category: 'electronics', limit: 10 }),
    );
